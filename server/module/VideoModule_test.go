...
func TestSave(t *testing.T) {
	...
	user, _ := client.User.Create().
		...
		Save(context)
	vim := NewVideoModule(repository.NewVideoRepository(context, client))
	t.Run("正常値", func(t *testing.T) {
		...
		video, err := vim.Save(request, user)
		if err != nil {
			t.Error(err)
		}
	...
	t.Run("title無しの場合", func(t *testing.T) {
		request := &model.SaveVideoRequest{
			...
			Value: &[]model.CodingSequence{},
		}
		_, err := vim.Save(request, user)
		expected := errors.New("video save request err title property is not set")
	...
	t.Run("Language無しの場合", func(t *testing.T) {
		request := &model.SaveVideoRequest{
			Header: struct {
				Title    string          "json:\"title\""
				Language *model.Language "json:\"language\""
		...
		_, err := vim.Save(request, user)
		expected := errors.New("video save request err lang property is not set")
		if fmt.Sprint(err) != fmt.Sprint(expected) {
			t.Errorf("チェックに失敗しました \n"+
				"expected:%v\n"+
				"actual  :%v", expected, err)
	...
	t.Run("coding sequence無しの場合(nil値)", func(t *testing.T) {
		request := &model.SaveVideoRequest{
			Header: struct {
				Title    string          "json:\"title\""
				Language *model.Language "json:\"language\""
		...
		_, err := vim.Save(request, user)
		expected := errors.New("video save request err coding sequence is not set")
		if fmt.Sprint(err) != fmt.Sprint(expected) {
			t.Errorf("チェックに失敗しました \n"+
				"expected:%v\n"+
				"actual  :%v", expected, err)
	...
	t.Run("coding sequence無しの場合(0配列)", func(t *testing.T) {
		request := &model.SaveVideoRequest{
			Header: struct {
				Title    string          "json:\"title\""
				Language *model.Language "json:\"language\""
...
func TestLoad(t *testing.T) {
	...
	t.Run("存在する場合", func(t *testing.T) {
		video, err := vim.Load(testId)
		if err != nil {
			t.Error(video)
		}
		t.Log("loaded video :", video.Header)
	})
	t.Run("存在しない場合", func(t *testing.T) {
		video, err := vim.Load(-1)
		if err != nil {
			t.Error(video)
		}
	})
