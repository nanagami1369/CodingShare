...
func (sc *SessionCreate) SetDateOfExpiry(t time.Time) *SessionCreate {
	sc.mutation.SetDateOfExpiry(t)
	return sc
}
...
func (sc *SessionCreate) check() error {
	if _, ok := sc.mutation.DateOfExpiry(); !ok {
		return &ValidationError{Name: "dateOfExpiry", err: errors.New(`ent: missing required field "dateOfExpiry"`)}
	}
...
func (sc *SessionCreate) createSpec() (*Session, *sqlgraph.CreateSpec) {
	...
	if value, ok := sc.mutation.DateOfExpiry(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
			Type:   field.TypeTime,
			Value:  value,
			Column: session.FieldDateOfExpiry,
		})
		_node.DateOfExpiry = value
	}
